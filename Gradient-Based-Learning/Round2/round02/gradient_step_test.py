...
It will perform tests for the following assignments:
    * Gradient Descent one feature: Given a set of test datasets (contained in the resources folder) it will generate
    an HTML report with the result of the solution for the assignment Gradient Descent one feature.
"""
import glob
import os
import sys
...
def __execute_test_case__(test_case, solution):
    test_input = test_case['input']
    expected_output = test_case['output']
    
    solution_output = solution(test_input.x, test_input.y, test_input.weight, test_input.lrate)
    test_result = TestResult(test_input, solution_output, expected_output, True)
    
    if not np.allclose(expected_output[0], solution_output[0]):
        test_result.succeeded = False
    elif not np.allclose(expected_output[1], solution_output[1]):
        test_result.succeeded = False
    return test_result

def test_gradient_step_one_feature(solution):
    sys.path.append(pwd)
    ...
    for test_file in test_files:
        with open(test_file, 'r') as fin:
            ...
            test_results.append(__execute_test_case__(test_case, solution))

    sys.path.remove(pwd)
    return generate_test_results(test_results)

def test_gradient_step(solution):
    sys.path.append(pwd)
...
def hidden_test_gradient_step_one_feature(solution):
    sys.path.append(pwd)
    test_files = glob.glob(os.path.join(resources_dir, 'gradient_step_one_feature_test_case_*.json'))
    test_results = []
    for test_file in test_files:
...
def hidden_test_gradient_step(solution):
    sys.path.append(pwd)
    test_files = glob.glob(os.path.join(resources_dir, 'gradient_step_test_case_*.json'))
    test_results = []
    for test_file in test_files:
